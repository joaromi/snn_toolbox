...
def layer_norm_J(model, config, norm_set=None, num_samples=None, divisions=1, free_GB=2, layers_to_plot=[], equiv_layers=[], **kwargs):
            ...
            for ix,group in enumerate(equiv_layers):
                if layer.name in group:
                    ...
                    for ref in refs: 
                        ...
                        shift = min(shift,scale_facs[ref][1]) # average
                    lmbda /= len(refs)
    ...
    for layer in model.layers:
        ...
        # Scale parameters
        parameters = layer.get_weights()
        lmbda = scale_facs[layer.name][0]
        shift = scale_facs[layer.name][1]
        inbound = get_inbound_layers_with_params(layer)
        if len(inbound) == 0:  # Input layer
            parameters_norm = [
            ...
            ws = np.sum(parameters[0],(0,1,2))*scale_facs[inbound[0].name][0]
            parameters_norm = [
            ...
            for inb in inbound:
                ...
                ws += np.sum(parameters[0][:,:,f_in],(0,1,2))*scale_facs[inb.name][0]
                offset += f_out
...
def channel_norm_J(model, config, norm_set=None, num_samples=None, divisions=1, free_GB=2, layers_to_plot=[], equiv_layers=[], **kwargs):
    ...
    if equiv_layers:
        for layer in model.layers:
            for ix,group in enumerate(equiv_layers):
                if layer.name in group:
                    refs = equiv_layers.pop(ix)
                    n_channels = len(scale_facs[refs[0]])
                    lbdas = np.zeros(n_channels)
                    shifts = np.zeros(n_channels)
                    for ref in refs: # average
                        lbdas += np.array(scale_facs[ref][0]) 
                        shifts += np.array(scale_facs[ref][1]) 
                    lbdas /= float(len(refs))
                    shifts /= float(len(refs))
    ...
    for layer in model.layers:
        ...
        # Scale parameters
        parameters = layer.get_weights()
        lbdas = np.array(scale_facs[layer.name][0])
        shifts = np.array(scale_facs[layer.name][1])
        inbound = get_inbound_layers_with_params(layer)
        ...
        if len(inbound) == 0:  # Input layer
            ...
            parameters_norm[0] = parameters[0]/(lbdas-shifts)
        elif len(inbound) == 1:
            ws = np.zeros(out_ch)
            for i in range(in_ch):
                ws+= parameters[0][:,:,i]*scale_facs[inbound[0].name][0][i]
                parameters_norm[0][:,:,i] *= (scale_facs[inbound[0].name][0][i]-scale_facs[inbound[0].name][1][i])/(lbdas-shifts)
            parameters_norm[1] = (parameters[1]+ws-shifts)/(lbdas-shifts)
        else:
            offset = 0  # Index offset at input filter dimension
            ws = []
            for inb in inbound:
                f_out = inb.filters  # Num output features of inbound layer
                ws_inb = np.zeros(f_out)
                for i in range(f_out):
                    ws_inb+= parameters[0][:,:,i+offset]*scale_facs[inb.name][0][i]
                    parameters_norm[0][:,:,i+offset] *= (scale_facs[inb.name][0][i]-scale_facs[inb.name][1][i])/(lbdas-shifts)
                ws.append(ws_inb)
                offset += f_out
            parameters_norm[1] = (parameters[1]+np.array(ws)-shifts)/(lbdas-shifts)
